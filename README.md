# WebTech
[Acesse o design aqui.](https://www.figma.com/design/Fdm4b9IXwf5PWrnIw2dYnK/Untitled?node-id=0-1&t=elhKBxdcuPRvc26N-1)

## Rodando 
Em caso de primeiro acesso o site se iniciará completamente vazio, com tema claro como padrão, podendo ser alterado no botão indicado no *Header*, caso contrário, e tenha cadastrado um item, haverá a persistência dos produtos.

### Relação de Afazeres:
1. Cauã Vitor: Estruturação de HTML e estilização de produto.
2. Arthur: Projetação e dinamicidade do site.
3. Daniel/Iarley/Júlio/Cauã/Arthur: Projetação básica de design em software de modelos.
### Componentes:
- Arthur Moreira Barros
- Cauã Vitor Costa da Silva
- Daniel dos Santos Soares
- Iarley de Lucena Rodrigues dos Santos
- Júlio César Amaral
